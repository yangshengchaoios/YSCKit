...
- (void)asyncPlayingWithPath:(NSString *)aFilePath  completion:(void(^)(NSError *error))completon{
     ...
     */
    WEAKSELF
    [self setupAudioSessionCategory:EM_AUDIOPLAYER isActive:YES];
    [EMAudioPlayerUtil asyncPlayingWithPath:aFilePath completion:^(NSError *error) {
...
// 开始录音
- (void)asyncStartRecordingWithFileName:(NSString *)fileName completion:(void(^)(NSError *error))completion {
    NSError *error = nil;
    ...
    [EMAudioRecorderUtil asyncStopRecordingWithCompletion:^(NSString *recordPath) {
        if (completion) {
            if (recordPath) {
                ...
                BOOL convertResult = [weakSelf convertWAV:recordPath toAMR:amrFilePath];
                if (convertResult) {
                    // 删除录的wav
                    NSFileManager *fm = [NSFileManager defaultManager];
                    [fm removeItemAtPath:recordPath error:nil];
                }
                completion(amrFilePath, [weakSelf recordingDuration], nil);
            }
            [weakSelf setupAudioSessionCategory:EM_DEFAULT isActive:NO];
        }
    }];
