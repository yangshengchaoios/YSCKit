...
- (void)updateConstraints {
    ...
    [subviews enumerateObjectsUsingBlock:^(UIView *view, NSUInteger idx, BOOL *stop) {
        ...
        if (idx > 0) {
            // Has previous
            NSLayoutConstraint *padding = [NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:subviews[idx - 1] attribute:NSLayoutAttributeBottom multiplier:1 constant:0.f];
...
- (void)updatePaddingConstraints {
    ...
    CGFloat subviewPadding = self.bezelSubViewsPadding;
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? subviewPadding : 0.f;
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
        if (self.effectView && [self.effectView.subviews isEqual:self]) {
